...
But there are questions: movement is slow, practical capability still under scrutiny.`,
    ...
    {
      id: 2,
      title: "OpenAI to build major data centre in India as part of its $500 billion “Stargate” project",
...
This reflects India’s rising importance in AI both in terms of user base and talent. Also signals big investment in AI infrastructure in the country.`,
    ...
    {
      id: 3,
      title: "TN SPARK” initiative: AI, robotics, coding training rolled out in Tamil Nadu govt schools",
      excerpt: `
    ...
    <>
      ...
      <section className="py-24 bg-[#0a0e1a]">
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            {BLOG_DATA.map(post => (
              <div key={post.id} className="bg-[rgba(30,41,59,0.3)] border border-[rgba(59,130,246,0.2)] rounded-2xl overflow-hidden transition-all duration-400 hover:-translate-y-2 hover:border-[#3b82f6] hover:shadow-xl hover:shadow-[rgba(59,130,246,0.2)] group">
                <div className="h-48 bg-gradient-to-r from-[#1e293b] to-[#334155] flex items-center justify-center text-[#3b82f6] text-5xl transition-all duration-300 group-hover:bg-gradient-to-r group-hover:from-[#3b82f6] group-hover:to-[#8b5cf6] group-hover:text-white">
                  ...
                  {post.category === "Technology" ? <FaRobot /> :
                    post.category === "Innovation" ? <FaTrophy /> : <FaBrain />}
                </div>
                <div className="p-6">
                  ...
                  <h3 className="text-xl font-bold mb-4 text-[#f1f5f9] transition-colors duration-300 group-hover:text-[#3b82f6]">{post.title}</h3>
                  <p className="text-[#94a3b8] mb-6 leading-relaxed">{post.excerpt}</p>
                  <div className="flex gap-2">
                    {post.tags.map((tag, index) => (
                      <span key={index} className="bg-[rgba(139,92,246,0.2)] text-[#8b5cf6] px-3 py-1 rounded-full text-xs font-semibold tracking-wider">
                        {tag}
                      </span>
                    ))}
                  </div>
